...
export default function CirclePlay() {
  ...
  if (!loggedIn) return <main className="login-screen"><div className="login-art" /><section className="login-panel"><Logo /><p className="eyebrow">SUA PRÓXIMA HISTÓRIA</p><h1>Filmes, séries e<br /><span>canais ao vivo.</span></h1><p className="login-copy">Entre para continuar assistindo suas histórias favoritas em qualquer tela.</p><form onSubmit={(event) => { event.preventDefault(); setLoggedIn(true); setLoginError('') }}><label>E-mail ou usuário<input className="focusable" required placeholder="voce@exemplo.com" /></label><label>Senha<div className="password-wrap"><input className="focusable" required minLength={4} type={showPassword ? 'text' : 'password'} placeholder="••••••••" /><button type="button" className="show-password focusable" onClick={() => setShowPassword(!showPassword)}>{showPassword ? 'Ocultar' : 'Mostrar'}</button></div></label>{loginError && <p className="error-message">{loginError}</p>}<button className="primary-button focusable" type="submit">Entrar <span>→</span></button></form><p className="login-footer">Ainda não tem uma conta? <button className="link-button focusable" type="button">Criar agora</button></p></section></main>
